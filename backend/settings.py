...
class _SearchCommonSettings(BaseSettings):
    ...
    role_information: str = Field(validation_alias="AZURE_OPENAI_SYSTEM_MESSAGE")
...
class _AzureCosmosDbMongoVcoreSettings(BaseSettings, DatasourcePayloadConstructor):
    model_config = SettingsConfigDict(
...
class _ElasticsearchSettings(BaseSettings, DatasourcePayloadConstructor):
    ...
    def construct_payload_configuration(self, *args, **kwargs):
        self.embedding_dependency = (
            ...
            else self._settings.azure_openai.extract_embedding_dependency()
        )
...
class _PineconeSettings(BaseSettings, DatasourcePayloadConstructor):
    ...
    @model_validator(mode="after")
    def set_authentication(self) -> Self:
        self.authentication = {"type": "api_key", "api_key": self.api_key}
...
class _AzureSqlServerSettings(BaseSettings, DatasourcePayloadConstructor):
    model_config = SettingsConfigDict(
        env_prefix="AZURE_SQL_SERVER_", env_file=DOTENV_PATH, extra="ignore"
    )
