...
SHOULD_STREAM = False if AZURE_OPENAI_STREAM.lower() == "true" else False
...
def should_use_data():
    ...
    if PROMPTFLOW_ENDPOINT and PROMPTFLOW_API_KEY:
        ...
        logging.debug("Using Promptflow")
        return True
...
async def complete_chat_request(request_body):
    if DATASOURCE_TYPE == "Promptflow":
        ...
        return format_pf_non_streaming_response(response, history_metadata)
    else:
...
async def stream_chat_request(request_body):
    # TODO: Implement streaming for Promptflow
    response = await send_chat_request(request_body)
    history_metadata = request_body.get("history_metadata", {})
